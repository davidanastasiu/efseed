...
#         super(DS, self).__init__()
    ...
    def read_dataset(self):
        ...
        if(self.is_prob_feature==1):
            ...
            self.z2 = 1.2*np.max(self.gm_means)
...
# use randomly chosen validation set            
                ...
                if (not np.isnan(self.sensor_data_norm1[i:i+self.lens]).any()) and (not np.isnan(self.R_data[i:i+self.lens]).any())  and (self.tag[i+self.train_days] <= a1 or a2 < self.tag[i+self.train_days] < 0 or 2 <= self.tag[i+self.train_days] <= 3 ): 
                    ...
                    i = self.trainX[self.trainX["datetime"]==point].index.values[0] - self.start_num - self.train_days
    ...
    def train_dataloader(self):
        ...
        if(self.is_over_sampling == 1):
            ...
            while ii < self.opt.train_volume*(1-self.oversampling/100) or jj < self.opt.train_volume*(self.oversampling/100):            
                ...
                a1 = -9
                a2 = -6
                if (np.mean(pre1) > self.z2):
                    a3 = self.os_h
                    max_index = np.argmax(pre1)
                a5 = self.iterval
                if (jj < self.opt.train_volume*(self.oversampling/100)) and (np.mean(pre1) > self.z2) and ( not np.isnan(self.sensor_data_norm1[i:i+self.lens]).any()) and (self.tag[i+self.train_days] <= a1 or a2 < self.tag[i+self.train_days] < 0):
                    if a3 > 0:
                        i = i + max_index -1
                        i = i - a3*a5
